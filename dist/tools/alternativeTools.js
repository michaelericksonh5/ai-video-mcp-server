...
import { configureFal, falHappyHorseImageToVideo, falHappyHorseReferenceToVideo, falSeedanceImageToVideo, falSeedanceReferenceToVideo, } from "../services/falClient.js";
function formatResult(result) {
    ...
    if (result.video.duration)
        lines.push(`**Duration**: ${result.video.duration}s`);
    if (result.video.fps)
        lines.push(`**FPS**: ${result.video.fps}`);
...
export function registerAlternativeTools(server) {
    // ── Happy Horse: Image to Video ───────────────────────────────────────────
    server.registerTool("veo_happy_horse_image_to_video", {
...
Returns: Animated video URL and metadata.`,
    ...
    server.registerTool("veo_happy_horse_reference_to_video", {
...
Returns: Video URL and metadata.`,
    ...
    server.registerTool("veo_seedance_image_to_video", {
...
Returns: Video URL and metadata.`,
    ...
    server.registerTool("veo_seedance_reference_to_video", {
...
//# sourceMappingURL=alternativeTools.js.map